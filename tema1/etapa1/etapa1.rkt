...
(define (get-women-iter wpref res)
  (if (null? wpref)
      res
      (get-women-iter (cdr wpref)
                      (append res (list (car (car wpref)))))))

(define (get-women wpref)
  (get-women-iter wpref null))
...
(define (find-preferable pref-list x y)
  (or (and (equal? x (car pref-list)) x)
      (and (equal? y (car pref-list)) y)
      (find-preferable (cdr pref-list) x y)))

(define (preferable? pref-list x y)
  (equal? x (find-preferable pref-list x y)))
