...
import Data.List (insertBy, sort)
import Data.Maybe
import TestPP
...
instance Eq a => Eq (BST a) where
  (==) BSTNil BSTNil = True
  (==) (BSTNod v1 l1 r1) (BSTNod v2 l2 r2) =
    (v1 == v2) && (l1 == l2) && r1 == r2
  (==) _ _ = False
...
instance Show a => Show (BST a) where
  show = showNode ""
    where
      showNode depth BSTNil = ""
      showNode depth (BSTNod v l r) =
        depth ++ show v ++ "\n" ++ showNode ("\t" ++ depth) l ++ showNode ("\t" ++ depth) r
...
instance Ord a => Ord (BST a) where
  ...
  (<=) BSTNil (BSTNod {}) = True
  (<=) (BSTNod {}) BSTNil = True
  (<=) node1 node2 = height node1 <= height node2
...
instance Invertible (BST a) where
  ...
  invert (BSTNod v l r) = BSTNod v r l
...
instance Functor BST where
  ...
  fmap f (BSTNod v l r) = BSTNod (f v) (fmap f l) (fmap f r)
...
instance Foldable BST where
  ...
  foldr f acc (BSTNod v r l) = foldr f acc2 l where
    acc1 = foldr f acc r
    acc2 = f v acc1
