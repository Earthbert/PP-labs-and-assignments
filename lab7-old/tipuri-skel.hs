...
data BST a = BSTNode {val :: a, l :: BST a, r :: BST a} | BSTNil deriving (Show)

insertElem :: (Ord a, Eq a) => BST a -> a -> BST a
insertElem BSTNil a = BSTNode a BSTNil BSTNil
insertElem bst@(BSTNode val l r) a
  | a == val = bst {val = a}
  | a < val = bst {l = insertElem l a}
  | otherwise = bst {r = insertElem r a}

findElem :: (Ord a, Eq a) => BST a -> a -> Maybe a
findElem BSTNil a = Nothing
findElem bst@(BSTNode val l r) a
  | a == val = Just a
  | a < val = findElem l a
...
inorder :: BST a -> [a]
inorder BSTNil = []
inorder bst@(BSTNode val l r) = inorder l ++ [val] ++ inorder r

size :: BST a -> Int
size BSTNil = 0
size bst@(BSTNode val l r) = size l + 1 + size r

height :: BST a -> Int
height BSTNil = 0
height bst@(BSTNode val l r) = max (height l) (height r) + 1

isBalanced :: (Ord a, Num a) => BST a -> Bool
isBalanced BSTNil = True
isBalanced bst@(BSTNode val l r) =
...
-}
data Tree a = TNode {v :: a, children :: [Tree a]} | TNill deriving (Show)
...
consElem elem (List l) = List (Atom elem : l)

consList :: NestedList a -> NestedList a -> NestedList a
consList list (List l) = List (list : l)

headList :: NestedList a -> NestedList a
headList (List l) = head l

tailList :: NestedList a -> NestedList a
tailList (List l) = List (tail l)

deepEqual :: Eq a => NestedList a -> NestedList a -> Bool
deepEqual (Atom elem1) (Atom elem2) = elem1 == elem2
deepEqual (List []) (List []) = True
deepEqual l1@(List list1) l2@(List list2) =
  deepEqual (headList l1) (headList l2) && deepEqual (tailList l1) (tailList l2)
deepEqual _ _ = False

flatten :: NestedList a -> [a]
flatten (Atom elem) = [elem]
flatten (List []) = []
flatten l = flatten (headList l) ++ flatten (tailList l)

check4 :: TestData
check4 =
  let l1 = consElem 1 emptyList
      l2 =
        consElem 2 $
          consList (consElem 1 $ consElem 1 emptyList) $
...
checkBonus :: TestData
checkBonus =
  let bfsNodes = bfs $ completeBinaryTree 1
