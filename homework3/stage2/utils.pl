...
editall :- forall(member(File, [
                             ccs, utils, testing, 'checker.pl', points1, points2
                         ]), edit(File)).
...
% neighbor/3 - construiește perechea de coordonate XN,YN vecină cu X,Y,
% pe direcția Dir
...
printBoard(Board) :-
    boardGetLimits(Board, Xm, Ym, XM, YM),
%    format("Limits: ~w ~w ~w ~w~n", [Xm, Ym, XM, YM]),
...
%                              , format("~w ~w: ~w~n", [X, Y, Tile])
                ...
                append(L1, [xtile(Y)], Line)
                ),
           Lines),
    reverse([HL | Lines], LR), foldl(append, LR, [], Tiles),
    W is XM - Xm + 2,
    printTiles(Tiles, W), format("~n").
...
printable(xtile(S), PP) :- !, findall([' ',' ',X,' ',' '], (member(R, [1,2,3,4,5]),
                                                        (   (R == 3) -> X = S; X = ' ')), PP).
printable(P, PP) :-
    findall(RowP,
