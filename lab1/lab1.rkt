...
(define (deep-rev L)
  {cond
    [(null? L) `()]
    [(not (list? (car L))) (append (deep-rev (cdr L))
                                   (list (car L)))]
...
(define (all-palindromes? n Bases)
  {cond
    [(null? Bases) true]
    [(palindrome? (num->base n (car Bases))) (all-palindromes? n (cdr Bases))]
    [else false]})
...
(define GivenBases `(2 3 4 5 6 7 8 9 10))

(define (how-many-bases-palindromes? n Bases)
  {cond [(null? Bases) 0]
        [(palindrome? (num->base n (car Bases))) (+ (how-many-bases-palindromes? n (cdr Bases)) 1)]
        [else (how-many-bases-palindromes? n (cdr Bases))]})

(define (first-b-pal start b)
  (cond [(> b (length GivenBases)) "Not Possible"]
        [(<= b (how-many-bases-palindromes? start GivenBases)) start]
        [else (first-b-pal (+ start 1) b)]))
...
(define (prefix-mover L)
  (if (palindrome? L) L
                      (prefix-mover(take L (- (length L) 1)))))

(define (suffix-mover L)
  (cond [(= 1 (length L)) L]
        [(if (> (length (prefix-mover L)) (length (suffix-mover (cdr L))))
            (prefix-mover L)
            (suffix-mover (cdr L)))]))

(define (longest-palindrome n)
  (list->num (suffix-mover (num->base n 10))))
