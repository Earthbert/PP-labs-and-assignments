...
(define (compute-length get-line-segment get-start-point get-end-point)
  (let* ((seg (get-line-segment)) (start (get-start-point seg)) (end (get-end-point seg))
        (x (abs (- (car end) (car start)))) (y (abs (- (cdr end) (cdr start)))))
    (sqrt (+ (* x x) (* y y)))))
...
(define (distance x y)
  (compute-length (lambda () (cons x y))
                  (lambda (seg) (car seg))
                  (lambda (seg) (cdr seg))))
...
(define (compute-perimeter points)
  (let compute-perimeter-iter ((L points) (sum 0))
        (if (null? (cdr L))
            (+ sum (distance (car L) (car points)))
            (compute-perimeter-iter (cdr L) (+ sum (distance (car L) (cadr L)))))))
...
(define (player-wins? candies)
  (letrec ((player-turn
            (lambda (n) 
                    (cond ((or (= 1 n) (= 2 n)) true)
                          ((= 0 n) false)
                          (else (or (opponent-turn (- n 1)) (opponent-turn (- n 2)))))))
          (opponent-turn
            (lambda (n)
...
(define (3-sequence-max numbers separator)
  (let*-values  (((pred) (lambda (x) (not (equal? x separator))))
                ((first-third rest1) (splitf-at numbers pred))
                ((second-third rest2) (splitf-at (cdr rest1) pred))
                ((last-third) (cdr rest2)))
                (max (sum first-third) (sum second-third) (sum last-third))))
...
(define (num-concat x y)
  (define (+ x y)
    (string->number (string-append  (number->string x)
                                    (number->string y))))
